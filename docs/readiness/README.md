...
| 7 | Restore drill | `docs/runbooks/restore-drill.md` against the latest archive in a temporary environment | dry run then apply both end `verified`; app boots on it; per-table counts match the source |
...
| 12 | Runbooks | go-live, release, phone-line-down, outbox-dead-letter, replay-exhausted, quarantine, legacy-import, patients, team, classifier-rules, restore-drill | each followed once end to end on staging |
