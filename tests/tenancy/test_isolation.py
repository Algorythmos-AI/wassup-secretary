...
from tests.tenancy.conftest import Seed, as_role

pytestmark = pytest.mark.db


def _call_ids(engine: Engine, role: str, clinics: list[uuid.UUID] | None) -> set[uuid.UUID]:
...
def test_each_role_sees_only_its_declared_clinic(engine: Engine, seed: Seed, role: str) -> None:
    assert _call_ids(engine, role, [seed.clinic_a]) == {seed.call_a}
    assert _call_ids(engine, role, [seed.clinic_b]) == {seed.call_b}


@pytest.mark.parametrize("role", ["app_core", "app_voice", "app_ops", "wassup_owner"])
def test_no_context_means_zero_rows(engine: Engine, seed: Seed, role: str) -> None:
    assert _call_ids(engine, role, None) == set()


def test_multi_clinic_context_sees_both(engine: Engine, seed: Seed) -> None:
    assert _call_ids(engine, "app_core", [seed.clinic_a, seed.clinic_b]) == {
        seed.call_a,
        seed.call_b,
    }


def test_cannot_write_into_another_clinic(engine: Engine, seed: Seed) -> None:
    with engine.connect() as conn, conn.begin():
...
def test_cannot_move_a_row_to_another_clinic(engine: Engine, seed: Seed) -> None:
    with engine.connect() as conn, conn.begin():
...
def test_owner_is_also_subject_to_rls(engine: Engine, seed: Seed) -> None:
    """FORCE ROW LEVEL SECURITY: even the table owner needs a clinic context."""
    assert _call_ids(engine, "wassup_owner", [seed.clinic_a]) == {seed.call_a}


def test_resolver_requires_agent_and_number_to_agree(engine: Engine, seed: Seed) -> None:
    with engine.connect() as conn, conn.begin():
...
def test_resolvers_are_granted_to_exactly_one_role(
    engine: Engine, role: str, function_call: str
) -> None:
    with engine.connect() as conn, conn.begin():
...
def test_least_privilege_grants(engine: Engine, role: str, statement: str) -> None:
    with engine.connect() as conn, conn.begin():
        as_role(conn, role, None)
        with pytest.raises(ProgrammingError, match="permission denied"):
...
@pytest.mark.parametrize("role", ["app_core", "app_voice", "app_ops"])
def test_audit_log_is_append_only(engine: Engine, seed: Seed, role: str) -> None:
    with engine.connect() as conn, conn.begin():
