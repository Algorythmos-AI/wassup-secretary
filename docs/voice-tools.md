...
| `{"matched": true, "patient_ref": "…", "verified": true}` | Exactly one living patient matches **and** the caller is ringing from the number on that patient's record. | The agent may refer to patient-specific facts. |
